test_that("inject_generated_notice adds notice after YAML", {
      ...
      suppressMessages(inject_generated_notice(path, "vignettes-raw/test.Rmd"))
...
test_that("inject_generated_notice handles file with no YAML", {
      ...
      suppressMessages(inject_generated_notice(path, "src.Rmd"))
...
test_that("add_to_rbuildignore is idempotent", {
      withr::with_tempdir({
            suppressMessages(add_to_rbuildignore("^vignettes-raw$"))
